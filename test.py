...
for i in open("log.txt","r"):
   ...
   algo = i.find("ice el disparo ")
  
   if(algo>0):
     aux = i[algo+15:]
     buffer = buffer+aux.rstrip('\n')
    #  buffer = buffer+aux
...
def verifyInvariants(buffer ):
	...
	while(True):
		...
		line = re.subn('T0(.*?)((TA(.*?)((T6(.*?)T7(.*?)T2(.*?)T3(.*?)T4)|(T2(.*?)T3(.*?)T5)|(T2(.*?)T5(.*?)T3)|(T5(.*?)T2(.*?)T3)))|((T8(.*?)((T13(.*?)T14(.*?)T9(.*?)T10(.*?)T11)|(T9(.*?)T10(.*?)T12)|(T9(.*?)T12(.*?)T10)|(T12(.*?)T9(.*?)T10)))))',
